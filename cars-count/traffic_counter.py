...
cap = cv2.VideoCapture('singlelane.mp4')
...
while True:
    ...
    if ret:  # if there is a frame continue with code
        ...
        cv2.rectangle(image, (0, 0), (250, 100), (0, 0, 0), -1)  # background rectangle for on-screen text

        cv2.putText(image, "Cars in Area: " + str(currentcars), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 0, 255), 1)

        cv2.putText(image, "Cars Crossed Up: " + str(carscrossedup), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 0, 255),
                    1)
        ...
        cv2.putText(image, 'Time: ' + str(round(framenumber / fps, 2)) + ' sec of ' + str(round(frames_count / fps, 2))
                    + ' sec', (0, 90), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 0, 255), 1)
